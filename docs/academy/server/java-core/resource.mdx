---
id: resource 
title: Polyglot Low-Code Resource 
...
To use a resource while coding in Java, some classes such as `org.netuno.tritao.Web` and `org.netuno.tritao.event.EventBase` provide the `resource` function, which initializes this resource. 
In the class where you want to use the `calc` resource, do:
...
You can view the complete project files in this [link](https://github.com/netuno-org/maven/tree/main/Calc/src/main/java/org/netuno/addon/calc/resource).
