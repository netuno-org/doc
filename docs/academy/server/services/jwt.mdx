...
On the frontend, you can install it with [Bun](https://bun.sh):

- `bun install @netuno/auth-client`

> Another frontend dependency manager such as PNPM, NPM, YARN, or another can be used.
...
In the ES-module build, `config()` is asynchronous. Await it before using the configured client:

```javascript
import _auth from '@netuno/auth-client';

await _auth.config({
    storage: 'local', // session is the default value.
