---
id: pdf
title: PDF
sidebar_label: PDF
---

import PolyglotCode from "@site/src/components/PolyglotCode";
import PolyglotPath from "@site/src/components/PolyglotPath";

Process customized PDF files.

## Introduction

Netuno generates PDF files with support for images, tables, fonts, and more, through the [PDF](/docs/library/resources/pdf) resource that allows a low-code abstraction of [iTextPDF](https://itextpdf.com/) for all the programming languages supported by Netuno.

The application that comes with Netuno called **demo** (demonstration application) contains many examples of possible code implementations in various languages, including a demonstration of exporting to PDF through the `export-pdf` service located at:

<PolyglotPath files={[
    `netuno/apps/demo/server/services/samples/%lang%/export-pdf`
]} />

## Initialization

Firstly, you need to create the object that will contain the context of the new document that is being created, by default the page size is A4.

When creating the document, you define where it will be generated.

If nothing is defined, it will be generated in the service's data output, for example:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfDocument = _pdf.newDocument()
            `
        },
        {
            lang: 'python',
            code: `
                pdfDocument = _pdf.newDocument()
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfDocument = _pdf.newDocument()
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfDocument = _pdf.newDocument()
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfDocument = _pdf.newDocument()
            `
        },
    ]}
/>

To generate a PDF file inside the application's `docs` folder, you can do it like this:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfDocument = _pdf.newDocument(_app.file("docs/file.pdf").output())
            `
        },
        {
            lang: 'python',
            code: `
                pdfDocument = _pdf.newDocument(_app.file("docs/file.pdf").output())
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfDocument = _pdf.newDocument(_app.file("docs/file.pdf").output())
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfDocument = _pdf.newDocument(_app.file("docs/file.pdf").output())
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfDocument = _pdf.newDocument(_app.file("docs/file.pdf").output())
            `
        },
    ]}
/>


If you are going to generate the document in a new file outside of the Application, for example:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfDocument = _pdf.newDocument(_os.file("/tmp/file.pdf").output())
            `
        },
        {
            lang: 'python',
            code: `
                pdfDocument = _pdf.newDocument(_os.file("/tmp/file.pdf").output())
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfDocument = _pdf.newDocument(_os.file("/tmp/file.pdf").output())
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfDocument = _pdf.newDocument(_os.file("/tmp/file.pdf").output())
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfDocument = _pdf.newDocument(_os.file("/tmp/file.pdf").output())
            `
        },
    ]}
/>

Or even in the application `storage`:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfDocument = _pdf.newDocument(_storage.filesystem("server", "file.pdf").output())
            `
        },
        {
            lang: 'python',
            code: `
                pdfDocument = _pdf.newDocument(_storage.filesystem("server", "file.pdf").output())
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfDocument = _pdf.newDocument(_storage.filesystem("server", "file.pdf").output())
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfDocument = _pdf.newDocument(_storage.filesystem("server", "file.pdf").output())
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfDocument = _pdf.newDocument(_storage.filesystem("server", "file.pdf").output())
            `
        },
    ]}
/>

> By default, the document is created in **A4** page format.

### Page Format

To customize the page format, simply indicate it when creating the document.

Creates a new document to output data from the service with pages in **letter** format:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfDocument = _pdf.newDocument(_pdf.pageSize("letter"))
            `
        },
        {
            lang: 'python',
            code: `
                pdfDocument = _pdf.newDocument(_pdf.pageSize("letter"))
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfDocument = _pdf.newDocument(_pdf.pageSize("letter"))
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfDocument = _pdf.newDocument(_pdf.pageSize("letter"))
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfDocument = _pdf.newDocument(_pdf.pageSize("letter"))
            `
        },
    ]}
/>

Creates a new document for a file in the application root with the **B3** page format:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfDocument = _pdf.newDocument(_app.file("file.pdf").output(), _pdf.pageSize("B3"))
            `
        },
        {
            lang: 'python',
            code: `
                pdfDocument = _pdf.newDocument(_app.file("file.pdf").output(), _pdf.pageSize("B3"))
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfDocument = _pdf.newDocument(_app.file("file.pdf").output(), _pdf.pageSize("B3"))
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfDocument = _pdf.newDocument(_app.file("file.pdf").output(), _pdf.pageSize("B3"))
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfDocument = _pdf.newDocument(_app.file("file.pdf").output(), _pdf.pageSize("B3"))
            `
        },
    ]}
/>

Creates a new document in the application's `storage` with the **legal** page format:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfDocument = _pdf.newDocument(_storage.filesystem("server", "file.pdf").output(), _pdf.pageSize("legal"))
            `
        },
        {
            lang: 'python',
            code: `
                pdfDocument = _pdf.newDocument(_storage.filesystem("server", "file.pdf").output(), _pdf.pageSize("legal"))
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfDocument = _pdf.newDocument(_storage.filesystem("server", "file.pdf").output(), _pdf.pageSize("legal"))
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfDocument = _pdf.newDocument(_storage.filesystem("server", "file.pdf").output(), _pdf.pageSize("legal"))
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfDocument = _pdf.newDocument(_storage.filesystem("server", "file.pdf").output(), _pdf.pageSize("legal"))
            `
        },
    ]}
/>

> See the documentation on [setting page sizes](/docs/library/resources/pdf/#pagesize).

PDF documents can be created with a custom page size definition. To do this, use `pageSize` by passing the value of the page width and height, for example:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                _pdf.pageSize(50, 30)
            `
        },
    ]}
/>

## HTTP Header

To create a service in the REST API that returns a PDF file as data output, you need to add the `Content-Type` parameter of the HTTP protocol to the header.

Example of how to set content as PDF:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                _header.contentType("pdf")
            `
        },
    ]}
/>

The `pdf` code is an abbreviation for:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                _header.contentType("application/pdf")
            `
        },
    ]}
/>

## Page Break

To create a new page, `areaBreak` is used:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                pdfDocument.add(
                    _pdf.areaBreak()
                )
            `
        },
    ]}
/>

## Paragraph

Add a new paragraph in the document:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                pdfDocument.add(
                    _pdf.paragraph("My text here...")
                        .setFontSize(15)
                )
            `
        },
    ]}
/>

To add a paragraph at a fixed position:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                pdfDocument.add(
                    pdfDocument.add(
                        _pdf.paragraph("My fixed text...")
                            .setFixedPosition(100, 500, 200)
                            .setFontSize(15)
                    )
                )
            `
        },
    ]}
/>

In the code above, in the `setFixedPosition` method, the first parameter is the X position (`100`), the second parameter is the Y position (`500`), and the third parameter is the width (`200`).

**Attention**: if Y is set to `0`, this means that it is at the bottom of the page, so to go up you need to increase the Y value, the higher the Y value, the more it will appear at the top, but if it is too high, beyond of the page length so it will not be visible.

> Documentation with all [iText Paragraph](https://api.itextpdf.com/iText/java/8.0.3/com/itextpdf/layout/element/Paragraph.html) object methods.

## Font

To define a native font we can do it as follows:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const helvetica = _pdf.font("helvetica")
                const helveticaBold = _pdf.font("helvetica-bold")
                const helveticaBoldOblique = _pdf.font("helvetica-boldoblique")
                const helveticaOblique = _pdf.font("helvetica-oblique")
            `
        },
        {
            lang: 'python',
            code: `
                helvetica = _pdf.font("helvetica")
                helveticaBold = _pdf.font("helvetica-bold")
                helveticaBoldOblique = _pdf.font("helvetica-boldoblique")
                helveticaOblique = _pdf.font("helvetica-oblique")
            `
        },
        {
            lang: 'ruby',
            code: `
                helvetica = _pdf.font("helvetica")
                helveticaBold = _pdf.font("helvetica-bold")
                helveticaBoldOblique = _pdf.font("helvetica-boldoblique")
                helveticaOblique = _pdf.font("helvetica-oblique")
            `
        },
        {
            lang: 'kotlin',
            code: `
                val helvetica = _pdf.font("helvetica")
                val helveticaBold = _pdf.font("helvetica-bold")
                val helveticaBoldOblique = _pdf.font("helvetica-boldoblique")
                val helveticaOblique = _pdf.font("helvetica-oblique")
            `
        },
        {
            lang: 'groovy',
            code: `
                def helvetica = _pdf.font("helvetica")
                def helveticaBold = _pdf.font("helvetica-bold")
                def helveticaBoldOblique = _pdf.font("helvetica-boldoblique")
                def helveticaOblique = _pdf.font("helvetica-oblique")
            `
        },
    ]}
/>

To define a customized font, we can use `storage`:


<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const viksiScript = _pdf.font(_storage.filesystem("server", "samples/export-pdf", "viksi-script.ttf"), true)
            `
        },
        {
            lang: 'python',
            code: `
                viksiScript = _pdf.font(_storage.filesystem("server", "samples/export-pdf", "viksi-script.ttf"), true)
            `
        },
        {
            lang: 'ruby',
            code: `
                viksiScript = _pdf.font(_storage.filesystem("server", "samples/export-pdf", "viksi-script.ttf"), true)
            `
        },
        {
            lang: 'kotlin',
            code: `
                val viksiScript = _pdf.font(_storage.filesystem("server", "samples/export-pdf", "viksi-script.ttf"), true)
            `
        },
        {
            lang: 'groovy',
            code: `
                def viksiScript = _pdf.font(_storage.filesystem("server", "samples/export-pdf", "viksi-script.ttf"), true)
            `
        },
    ]}
/>

We can also use a file inside the application, as this example shows that the font is inside the `fonts` folder in the application root:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const roboto = _pdf.font(_app.file("fonts/roboto.ttf"), true)
            `
        },
        {
            lang: 'python',
            code: `
                roboto = _pdf.font(_app.file("fonts/roboto.ttf"), true)
            `
        },
        {
            lang: 'ruby',
            code: `
                roboto = _pdf.font(_app.file("fonts/roboto.ttf"), true)
            `
        },
        {
            lang: 'kotlin',
            code: `
                val roboto = _pdf.font(_app.file("fonts/roboto.ttf"), true)
            `
        },
        {
            lang: 'groovy',
            code: `
                def roboto = _pdf.font(_app.file("fonts/roboto.ttf"), true)
            `
        },
    ]}
/>


Or use a file in any path on the system, so in this example the font is inside the `fonts` folder which is inside the system's temporary files folder:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const roboto = _pdf.font(_os.file("/tmp/fonts/roboto.ttf"), true)
            `
        },
        {
            lang: 'python',
            code: `
                roboto = _pdf.font(_os.file("/tmp/fonts/roboto.ttf"), true)
            `
        },
        {
            lang: 'ruby',
            code: `
                roboto = _pdf.font(_os.file("/tmp/fonts/roboto.ttf"), true)
            `
        },
        {
            lang: 'kotlin',
            code: `
                val roboto = _pdf.font(_os.file("/tmp/fonts/roboto.ttf"), true)
            `
        },
        {
            lang: 'groovy',
            code: `
                def roboto = _pdf.font(_os.file("/tmp/fonts/roboto.ttf"), true)
            `
        },
    ]}
/>

> Documentation with all [iText Font](https://api.itextpdf.com/iText/java/8.0.3/com/itextpdf/kernel/font/PdfFont.html) object methods.

To use the font in texts, the font constant must be passed to the paragraph, like this:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                pdfDocument.add(
                    _pdf.paragraph("Using bold font.")
                        .setFont(helveticaBold)
                        .setFontSize(15)
                )
            `
        },
    ]}
/>

## Image

Inserting images into the programmatically generated PDF document is done in this way:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                pdfDocument.add(
                    _pdf.image(_storage.filesystem("server", "samples/export-pdf", "logo.png"))
                        .scaleAbsolute(120, 36)
                )
            `
        },
    ]}
/>

Note that the `scaleAbsolute` method is very useful for resizing and adjusting the image size.

Alternatively, we can use a file within the application, as in this example:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const imageLogo = _pdf.image(_app.file("public/images/logo.png")
            `
        },
        {
            lang: 'python',
            code: `
                imageLogo = _pdf.image(_app.file("public/images/logo.png")
            `
        },
        {
            lang: 'ruby',
            code: `
                imageLogo = _pdf.image(_app.file("public/images/logo.png")
            `
        },
        {
            lang: 'kotlin',
            code: `
                val imageLogo = _pdf.image(_app.file("public/images/logo.png")
            `
        },
        {
            lang: 'groovy',
            code: `
                def imageLogo = _pdf.image(_app.file("public/images/logo.png")
            `
        },
    ]}
/>

Or use a file in any path on the system, as in this example:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const imageLogo = _pdf.image(_os.file("/tmp/images/pdf-logo.png")
            `
        },
        {
            lang: 'python',
            code: `
                imageLogo = _pdf.image(_os.file("/tmp/images/pdf-logo.png")
            `
        },
        {
            lang: 'ruby',
            code: `
                imageLogo = _pdf.image(_os.file("/tmp/images/pdf-logo.png")
            `
        },
        {
            lang: 'kotlin',
            code: `
                val imageLogo = _pdf.image(_os.file("/tmp/images/pdf-logo.png")
            `
        },
        {
            lang: 'groovy',
            code: `
                def imageLogo = _pdf.image(_os.file("/tmp/images/pdf-logo.png")
            `
        },
    ]}
/>

To add an image in a fixed position:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                pdfDocument.add(
                    imageLogo
                        .scaleAbsolute(120, 36)
                        .setFixedPosition(100, 500)
                )
            `
        },
    ]}
/>

Since above, in the `setFixedPosition` method, the first parameter is the X position (`100`), the second parameter is the Y position (`500`).

**Attention**: if Y is set to 0, this means that it is at the bottom of the page, so to go up you need to increase the Y value, the higher the Y value, the more it will appear at the top, but if it is too high, beyond of the page length so it will not be visible.

> Documentation with all [iText Image](https://api.itextpdf.com/iText/java/8.0.3/com/itextpdf/layout/element/Image.html) object methods.

## Table

It is possible to create tables, which allows you to present a set of data in a more readable way.

The number of columns in the table is defined with the parameter: `_pdf.table(numberOfColumns)`

In the example below, the `numberOfColumns` is the number `1`, that is, one column.

As we have added two cells in the table, so the second cell goes to the next row.

Repair:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                pdfDocument.add(
                    _pdf.table(1)
                        .addCell(
                            _pdf.cell()
                                .add(
                                    _pdf.paragraph("Person")
                                        .setFont(helvetica)
                                        .setFontSize(10)
                                )
                                .setBorder(_pdf.border("solid", 2))
                                .setBackgroundColor(_pdf.colorRGB(1, 0, 0))
                        )
                        .addCell(
                            _pdf.cell()
                                .add(
                                    _pdf.paragraph("Nicole")
                                        .setFont(helvetica)
                                        .setFontSize(10)
                                )
                                .setBackgroundColor(_pdf.colorGray(0.5))
                        )
                )
            `
        },
    ]}
/>

With the `addCell` method you can insert the definition of cells with their respective content.

> Documentation with all [iText Cell](https://api.itextpdf.com/iText/java//8.0.3/com/itextpdf/layout/element/Cell.html) object methods.

## Close and Save the Document

The document is only fully saved or generated, only when the `close` method is called, as follows:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                pdfDocument.close()
            `
        },
    ]}
/>

Where the document is generated is defined at initialization, so it is at document initialization that we define where the final document will be generated, whether it will be in the service's data output (`output`) or in a file.

## Edit File

You can edit an existing PDF document, for example:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfDocument = _pdf.openDocument(_app.file("file.pdf"))
            `
        },
        {
            lang: 'python',
            code: `
                pdfDocument = _pdf.openDocument(_app.file("file.pdf"))
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfDocument = _pdf.openDocument(_app.file("file.pdf"))
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfDocument = _pdf.openDocument(_app.file("file.pdf"))
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfDocument = _pdf.openDocument(_app.file("file.pdf"))
            `
        },
    ]}
/>

The final generated document will be sent in the service's data output.

> It is very useful to use a PDF as a base, as if it were a template.

Example for editing a document in the application `storage`:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfDocument = _pdf.openDocument(_storage.filesystem('server', 'file.pdf'))
            `
        },
        {
            lang: 'python',
            code: `
                pdfDocument = _pdf.openDocument(_storage.filesystem('server', 'file.pdf'))
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfDocument = _pdf.openDocument(_storage.filesystem('server', 'file.pdf'))
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfDocument = _pdf.openDocument(_storage.filesystem('server', 'file.pdf'))
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfDocument = _pdf.openDocument(_storage.filesystem('server', 'file.pdf'))
            `
        },
    ]}
/>

Uses a document as a base but saves changes to a new PDF file:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfDocument = _pdf.openDocument(
                    _app.file("document-base.pdf"),
                    _app.file("document-final.pdf")
                )
            `
        },
        {
            lang: 'python',
            code: `
                pdfDocument = _pdf.openDocument(
                    _app.file("document-base.pdf"),
                    _app.file("document-final.pdf")
                )
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfDocument = _pdf.openDocument(
                    _app.file("document-base.pdf"),
                    _app.file("document-final.pdf")
                )
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfDocument = _pdf.openDocument(
                    _app.file("document-base.pdf"),
                    _app.file("document-final.pdf")
                )
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfDocument = _pdf.openDocument(
                    _app.file("document-base.pdf"),
                    _app.file("document-final.pdf")
                )
            `
        },
    ]}
/>

Example using a document in `storage` as a base but saving it in another:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfDocument = _pdf.openDocument(
                    _storage.filesystem("server", "agreement-base.pdf"),
                    _storage.filesystem("server", "agreement-final.pdf")
                )
            `
        },
        {
            lang: 'python',
            code: `
                pdfDocument = _pdf.openDocument(
                    _storage.filesystem("server", "agreement-base.pdf"),
                    _storage.filesystem("server", "agreement-final.pdf")
                )
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfDocument = _pdf.openDocument(
                    _storage.filesystem("server", "agreement-base.pdf"),
                    _storage.filesystem("server", "agreement-final.pdf")
                )
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfDocument = _pdf.openDocument(
                    _storage.filesystem("server", "agreement-base.pdf"),
                    _storage.filesystem("server", "agreement-final.pdf")
                )
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfDocument = _pdf.openDocument(
                    _storage.filesystem("server", "agreement-base.pdf"),
                    _storage.filesystem("server", "agreement-final.pdf")
                )
            `
        },
    ]}
/>

With the document loaded, we can add content, and when closing with the `close` method at the end, the final document is generated with the new content added programmatically.

Example of a service that uses an existing PDF as a template to generate a PDF file output with dynamic content added:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                _header.contentType("pdf")

                const pdfDocument = _pdf.openDocument(_app.file("template-1.pdf"))

                pdfDocument.add(
                    _pdf.paragraph("My dynamic content.")
                        .setFixedPosition(37, 730, 100)
                        .setFontSize(15)
                )

                pdfDocument.close()
            `
        },
        {
            lang: 'python',
            code: `
                _header.contentType("pdf")

                pdfDocument = _pdf.openDocument(_app.file("template-1.pdf"))

                pdfDocument.add(
                    _pdf.paragraph("My dynamic content.")
                        .setFixedPosition(37, 730, 100)
                        .setFontSize(15)
                )

                pdfDocument.close()
            `
        },
        {
            lang: 'ruby',
            code: `
                _header.contentType("pdf")

                pdfDocument = _pdf.openDocument(_app.file("template-1.pdf"))

                pdfDocument.add(
                    _pdf.paragraph("My dynamic content.")
                        .setFixedPosition(37, 730, 100)
                        .setFontSize(15)
                )

                pdfDocument.close()
            `
        },
        {
            lang: 'kotlin',
            code: `
                _header.contentType("pdf")

                val pdfDocument = _pdf.openDocument(_app.file("template-1.pdf"))

                pdfDocument.add(
                    _pdf.paragraph("My dynamic content.")
                        .setFixedPosition(37, 730, 100)
                        .setFontSize(15)
                )

                pdfDocument.close()
            `
        },
        {
            lang: 'groovy',
            code: `
                _header.contentType("pdf")

                def pdfDocument = _pdf.openDocument(_app.file("template-1.pdf"))

                pdfDocument.add(
                    _pdf.paragraph("My dynamic content.")
                        .setFixedPosition(37, 730, 100)
                        .setFontSize(15)
                )

                pdfDocument.close()
            `
        },
    ]}
/>


## Read File

To read a PDF document, extracting the text information.

We use the `extract` method, which obtains all the data from the PDF file in a JSON object.

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const pdfData = _pdf.extract(_app.file("file-1.pdf"))

                _out.json(pdfData)
            `
        },
        {
            lang: 'python',
            code: `
                pdfData = _pdf.extract(_app.file("file-1.pdf"))

                _out.json(pdfData)
            `
        },
        {
            lang: 'ruby',
            code: `
                pdfData = _pdf.extract(_app.file("file-1.pdf"))

                _out.json(pdfData)
            `
        },
        {
            lang: 'kotlin',
            code: `
                val pdfData = _pdf.extract(_app.file("file-1.pdf"))

                _out.json(pdfData)
            `
        },
        {
            lang: 'groovy',
            code: `
                def pdfData = _pdf.extract(_app.file("file-1.pdf"))

                _out.json(pdfData)
            `
        },
    ]}
/>

The returned JSON has the key `content` with all the text content of the PDF file, and has the key `metadata` with the various properties of the PDF file.

Here is an example of the JSON returned by `extract`:

```json
{
  "content": "All text content of the PDF file goes here...",
  "metadata":{
    "Content-Type":"application/pdf",
    "dc:format":"application/pdf; version=1.4",
    "dc:title":"Linux 1",
    "pdf:PDFVersion":"1.4",
    "pdf:charsPerPage":"580",
    "pdf:containsDamagedFont":"false",
    "pdf:containsNonEmbeddedFont":"false",
    "pdf:docinfo:producer":"Skia/PDF m125 Google Docs Renderer",
    "pdf:docinfo:title":"File 1",
    "pdf:encrypted":"false",
    "xmpTPg:NPages":"3",
    ...
  }
}
```

There are many other data that are obtained in the `metadata` but that were omitted in the `...` so the example does not become too long.

## Conversion

It is possible to programmatically convert PDF to text or even HTML.

### PDF to Text

We can extract the entire content of the PDF file in plain text using the method:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                _pdf.toText(...)
            `
        },
    ]}
/>

Example of how to use it to read a file in the application root:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                _out.print(_pdf.toText(_app.file("my-document.pdf")))
            `
        },
    ]}
/>

Or using a file in the application's `storage`:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                _out.print(_pdf.toText(_storage.filesystem("server", "my-document.pdf")))
            `
        },
    ]}
/>

### PDF to HTML

We can extract the entire content of the PDF file into HTML using the method:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                _pdf.toHTML(...)
            `
        },
    ]}
/>

Example of how to use it to read a file in the application root:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                _out.print(_pdf.toHTML(_app.file("my-document.pdf")))
            `
        },
    ]}
/>

Or using a file in the application's `storage`:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                _out.print(_pdf.toHTML(_storage.filesystem("server", "my-document.pdf")))
            `
        },
    ]}
/>

### HTML para PDF

It is possible to generate PDF files from HTML code using the following method:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                _pdf.fromHTML(...)
            `
        },
    ]}
/>

Example of how to use it to generate a download file in the service:

<PolyglotCode
    codes={[
        {
            lang: 'javascript',
            code: `
                const icon = _app.file("public/images/icon.png");

                const pdfFile = _pdf.fromHTML(\`
                    <html>
                        <head>
                            <link href="https://fonts.googleapis.com/css2?family=Limelight&family=Roboto" rel="stylesheet">
                            <style>
                                * {
                                    font-family: Roboto;
                                }
                                div.sidebar {
                                    position: fixed;
                                    top: 0;
                                    bottom: 0;
                                    left: 0;
                                    width: 100px;
                                    background: #1B5886;
                                }
                                div.sidebar > div {
                                    position: absolute;
                                    top: 350px;
                                    transform: rotate(-90deg) translate(-50%, -34px);
                                    font-family: Limelight;
                                    font-size: 40px;
                                    color: #89CFF1;
                                }
                                main {
                                    padding-left: 150px;
                                }
                                h1 {
                                    margin: 0 auto;
                                    text-align: center;
                                    font-family: Limelight;
                                    color: #BA250E;
                                }
                                table {
                                    margin: 0 auto;
                                    border: 1px solid black;
                                    border-spacing: 0;
                                    border-collapse: collapse;
                                }
                                table > thead {
                                    background: #40712A;
                                    color: #fff;
                                }
                                table td, table th {
                                    padding: 5px;
                                    text-align: center;
                                }
                                table > tbody > tr:nth-child(even) {
                                    background: #e1e1e1;
                                }
                                a {
                                    font-family: Limelight;
                                    font-size: 16px;
                                    font-weight: bold;
                                    text-decoration: none;
                                    color: #1E5FB5;
                                }
                            </style>
                        </head>
                        <body>
                            <div class="sidebar">
                                <div>Demonstration</div>
                            </div>
                            <main>
                                <h1>PDF from HTML</h1>
                                <div style="text-align: center; margin: 30px 0;">
                                    <img width="50" src="\${icon.uri()}">
                                </div>
                                <p>
                                    This is an example HTML code for generating a PDF file.
                                    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nam
                                    tellus arcu, pharetra quis condimentum eget, dapibus convallis metus.
                                </p>
                                <table>
                                    <thead>
                                        <tr>
                                            <th>Name</th>
                                            <th>Age</th>
                                        </tr>
                                    </thread>
                                    <tbody>
                                        <tr>
                                            <td>Amelia Raine</td>
                                            <td>23</td>
                                        </tr>
                                        <tr>
                                            <td>Jack Butler</td>
                                            <td>38</td>
                                        </tr>
                                    </tbody>
                                </table>
                                <hr style="margin-top: 50px; height: 1px; background: #50B6C6; border: none;"/>
                                <div>
                                    <a href="https://netuno.org" target="_blank">Netuno.org</a>
                                </div>
                            </main>
                        </body>
                    </html>
                \`);

                _header.contentTypePDF();
                _out.copy(pdfFile.input());
            `
        },
    ]}
/>

## Videos

The following video demonstrates how to generate PDF files programmatically:

<iframe width="100%" height="420" src="https://www.youtube.com/embed/OyUzHr9hi0c?si=eTecDP0i0P_9rhG4" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
