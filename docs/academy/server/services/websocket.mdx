...
Next, you can start the connection:
...
const listenerRef = _ws.addListener({
    method: 'GET', // Optional; omit it to listen for every method.
    service: 'path/of/my/service/here',
    success: (data) => {
        console.log('Response from the service executed via WebSocket:', data);
    },
    fail: (error)=> {
        console.log('The service executed via WebSocket failed:', error);
    }
});
```
...
```js
_ws.sendService({
    method: 'GET', // Optional; requests default to GET when omitted.
    service: 'path/of/my/service/here',
    data: {
        message: 'Hi...'
    }
