...
When inserting or changing information, the _save_ and _saved_ actions are executed:

- `save`: Before executing the command in the database.
- `saved`: After executing the command in the database.

When **removing** or **deleting** information, the _remove_ and _removed_ actions are executed.
...
- `_db.insert(...)`, `_db.form(...).insert()`, `_db.insertIfNotExists(...)`, `_db.insertMany(...)`, among others.

- `_db.update(...)`, `_db.form(...).update()`, `_db.updateMany(...)`, among others.
- `_db.delete(...)`, `_db.form(...).delete()`, `_db.deleteMany(...)`, among others.
...
> See the [DataItem](/docs/library/resources/dataItem) resource documentation for more details on what
...
<PolyglotPath files={[
    'my_app/server/actions/my_form/save'
]} />
...
<PolyglotPath files={[
    'my_app/server/actions/my_form/saved'
]} />
...
<PolyglotCode
    file="my_app/server/actions/my_form/save"
    codes={[
        {
            lang: 'javascript',
            code: `
...
<PolyglotCode
    file="my_app/server/actions/my_form/saved"
    codes={[
        {
            lang: 'javascript',
            code: `
...
<PolyglotPath files={[
    'my_app/server/actions/my_form/setup_save',
    'my_app/server/actions/my_form/setup_saved'
]} />
...
<PolyglotPath files={[
    'my_app/server/actions/my_form/remove'
]} />
...
<PolyglotPath files={[
    'my_app/server/actions/my_form/removed'
]} />
...
<PolyglotCode
    file="my_app/server/actions/my_form/removed"
    codes={[
        {
            lang: 'javascript',
            code: `
        ...
        },
        {
            lang: 'python',
            code: `
                if _dataItem.isStatusAsError() and _dataItem.isStatusAsRelations():
                    _log.error(
                        "Could not remove because there is a relationship with table "
                        + _dataItem.getRelationTable() +" using ID " + _dataItem.getRelationItem().getInt("id")
                    )
...
<PolyglotCode
    file="my_app/server/actions/my_form/remove"
    codes={[
        {
            lang: 'javascript',
            code: `
...
<PolyglotPath files={[
    'my_app/server/actions/my_form/setup_remove',
    'my_app/server/actions/my_form/setup_removed'
]} />
