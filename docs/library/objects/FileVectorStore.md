...
| **embedding** | _[Values](/docs/library/objects/Values)_ | Query vector to compare against stored documents. |
| **topK** | _int_ | Maximum number of results to return. |
| **filter** | _[Values](/docs/library/objects/Values)_ | Metadata object to filter results. Only documents whose metadata contains all equal key-value pairs are returned. Can be null to disable filtering. |
