---
id: resource 
title: Recurso Poliglota Low-Code 
sidebar_label: Recurso 
sidebar_position: 5
---
...
Para usar um recurso no código Java, algumas classes como `org.netuno.tritao.Web`
e `org.netuno.tritao.event.EventBase` fornecem a função `resource`, que inicializa este recurso.
Na classe que você quiser que use o recurso `calc`, faça:

```java
import org.netuno.addon.calc.resource.CalcResource;

// ...
    CalcResource calc = resource(CalcResource.class);
    calc.set(10);
    calc.plus(120);
...
Você pode ver os arquivos completos do projeto nesse [link](https://github.com/netuno-org/maven/tree/main/Calc/src/main/java/org/netuno/addon/calc/resource).
