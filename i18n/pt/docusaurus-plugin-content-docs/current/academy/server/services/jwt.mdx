...
No frontend pode realizar a instalação com o [Bun](https://bun.sh):

- `bun install @netuno/auth-client`

> Pode ser utilizado outro gestor de dependências de frontend como PNPM, NPM, YARN, ou outro.
...
Na versão ES module, `config()` é assíncrono. Aguarde a sua conclusão antes de utilizar o cliente configurado:
