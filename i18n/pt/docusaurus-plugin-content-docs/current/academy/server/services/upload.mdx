...
<PolyglotCode
    codes={[
        ...
        },
        {
            lang: 'ruby',
            code: `
                meu_arquivo = _req.getFile("imagem")
                if !meu_arquivo.nil? && (
                    meu_arquivo.isExtension("jpeg") ||
                    meu_arquivo.isExtension("jpg") ||
                    meu_arquivo.isExtension("png")
                )
                    minha_imagem = _image.init(meu_arquivo)
                        .resize(200, 200)
                        .file(meu_arquivo.baseName() +".jpg", "jpeg")
                    _header.contentTypeJPG()
                    _out.copy(minha_imagem)
                end
            `
        },
        {
...
<PolyglotCode
    codes={[
                ...
                if meuArquivo is not None:
                    _out.json(
                        _val.map()
                            ...
                            .set("sizeMB", meuArquivo.sizeMB())
                            .set("sizeGB", meuArquivo.sizeGB())
                    )
                else:
                    _out.json(
                        _val.map()
        ...
        },
        {
            lang: 'ruby',
            code: `
                meu_arquivo = _req.getFile("file")
                if meu_arquivo.nil?
                    _out.json(
                        _val.map()
                            .set("result", true)
                            .set("name", meu_arquivo.name())
                            .set("sizeBytes", meu_arquivo.size())
                            .set("sizeKB", meu_arquivo.sizeKB())
                            .set("sizeMB", meu_arquivo.sizeMB())
                            .set("sizeGB", meu_arquivo.sizeGB())
                    )
                else
                    _out.json(
                        _val.map()
...
<PolyglotCode
    file="services/produto/imagem/post"
    codes={[
        {
            lang: 'javascript',
            code: `
...
Repare que o recurso `_storage` ([Storage](/docs/library/resources/storage)) é utilizado para obter o arquivo:
...
Na base de dados fica armazenado apenas o nome do arquivo, sendo que através do `_storage.database` podemos
obter o arquivo em si.
...
> O recurso [Storage](/docs/library/resources/storage) é utilizado para obter o caminho de pasta e de arquivo:
    ...
    codes={[
        {
            ...
            code: `
                _storage.filesystem(
                    ...
                    "upload", // Nome ou caminho da subpasta interna.
...
<PolyglotCode
    ...
    codes={[
        ...
        },
        {
            lang: 'ruby',
            code: `
                arquivo_nome = _req.getString("arquivo")
...
<PolyglotCode
    ...
    codes={[
        ...
        },
        {
            ...
            code: `
                arquivo_nome = _req.getString("arquivo")
                arquivo = _app.file("temp/#{arquivo_nome}")
