...
<a target="_blank" href="https://github.com/hierynomus/sshj/tree/master">SSHJ</a>, que é o que permite ao Netuno
fornecer estas funcionalidadas a todas linguagens de script através do recurso `_ssh`.
...
Verifique se já não existe configurações para o SSH, se não existir ainda então no fim do arquivo de
configuração (em formato JSON) adicione os parâmetros de configuração do SSH:
...
Para adicionar mais configurações de conexão SSH basta repetir o bloco de configuração padrão e trocar o nome
`default` por um outro nome à sua escolha, como por exemplo:
...
Uma configuração semelhante ao exemplo acima vai permitir utilizar a conta default quando for chamada da
seguinte forma no código:
